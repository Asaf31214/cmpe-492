...
def load_data_dictionary() -> str:
    """Load the clinical data dictionary as context."""
...
def build_system_prompt(data_dict: str) -> str:
...
## Clinical Data Dictionary Reference

{data_dict}

## Your Task

Predict whether the surgery will be successful (stone-free) or not based on preoperative features.
...
def main():
    results_file = DATA_DIR / "llm_predictions_qwen_v3.csv"
    
    # Check if predictions already exist
    if results_file.exists():
        print(f"Found existing predictions: {results_file}")
        results_df = pl.read_csv(results_file)
    else:
        df = pl.read_csv(DATA_DIR / "processed.csv")

        # Load data dictionary as context
        data_dict = load_data_dictionary()
        print(f"Loaded data dictionary ({len(data_dict)} chars)")

        system_prompt = build_system_prompt(data_dict)

        results = []

        print("Running LLM predictions with qwen3.5:4b (with data dictionary context)...")
        ...
        for idx, row in tqdm(enumerate(df.iter_rows(named=True)), total=len(df), desc="Predicting"):
            ...
            try:
                ...
                print(f"\nRow {idx}: pred={pred}, actual={row['SONUÇ-2']}, reasoning={reasoning[:80]}...")
    ...
    y_true = [int(x) for x in valid_results["actual"].to_list()]
    y_pred = [int(x) for x in valid_results["predicted"].to_list()]
    ...
    print(cm)
    print("\nPredictions loaded from: data/tabular/llm_predictions_qwen_v3.csv")
